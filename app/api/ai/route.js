...
export default function AAVMDashboard() {
  ...
  const handleRegenerateTitle = async (articleId) => {
    const article = articles.find(a => a.id === articleId);
    if (!article) return;

    if (!article.aiSummary) {
      alert('Please generate an AI summary first before regenerating the title.');
      return;
    }

    setArticles(prev => prev.map(a => 
      a.id === articleId 
        ? { ...a, regeneratingTitle: true }
        : a
    ));
            ...
            <div key={article.id} className="bg-white border border-gray-200 rounded-lg p-6 hover:shadow-md transition-shadow">
              <div className="flex justify-between items-start mb-4">
                <div className="flex-1">
                  ...
                  {article.regeneratedTitle && (
                    <div className="mb-2">
                      <h4 className="text-md font-semibold text-blue-900 bg-blue-50 p-2 rounded">
                        📝 AAVM Title: {article.regeneratedTitle}
                      </h4>
                    </div>
                  )}
